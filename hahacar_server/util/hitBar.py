            ...
            if lb == cat:
                ...
                if checkID == objID:
                    ...
                    if self._inRealm(oldPt, self.realmIn):
                        self.direction = (
                                    self.direction + np.array([pt[0] - oldPt[0], pt[1] - oldPt[1]], dtype=np.float32));
                        self.direction = self.direction / np.linalg.norm(self.direction);
                        self.speedInFrame.append(
                            np.linalg.norm(np.array([pt[0] - oldPt[0], pt[1] - oldPt[1]], dtype=np.float32)).item()
