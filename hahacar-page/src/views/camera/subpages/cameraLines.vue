<template>
  ...
  export default {
    ...
    data() {
      return {
        cameraId: -1,
        // 检测线数组，每条线包含名称、坐标、关联地图点、ID，以及 marker（如果存在）
        lines: [],
        // 当前正在绘制的线条数据
        currentLine: { points: [] },
        isDrawing: false,
    ...
    methods: {
      ...
      formatLinePoints(points) {
        return points && points.length === 4
          ? `${Math.round(points[0])},${Math.round(points[1])} -> ${Math.round(points[2])},${Math.round(points[3])}`
          : ''
      },
